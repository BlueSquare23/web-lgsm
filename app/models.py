...
class Job(db.Model):
    ...
    def __repr__(self):
        return f"<User(id={self.id}, server_id='{self.server_id}', command='{self.command}', comment='{self.comment}', expression='{self.expression}', date_created='{self.date_created}')>"

    def __str__(self):
        return f"User {self.username} (ID: {self.id}, Command: {self.command}, Comment: {self.comment}, Expression: {self.expression}, Created: {self.date_created})"
