...
def test_setup(app, client):
    with client:
        ...
        for char in bad_chars:
            response = client.post('/setup', data={'username':char, 'password1':PASSWORD, 'password2':PASSWORD})
            test_username_bad_chars(response)

        # Test passwords don't match.
        response = client.post('/setup', data={'username':USERNAME, 'password1':PASSWORD, 'password2':'blah'})
        # No redirect on setup.
        assert response.status_code == 400
        assert b"Passwords don&#39;t match!" in response.data

        # Test password too short.
        response = client.post('/setup', data={'username':USERNAME, 'password1':'Ab3$', 'password2':'Ab3$'})
        # No redirect on setup.
        assert response.status_code == 400
        assert b"Password is too short!" in response.data

        # Finally, create real test user.
        response = client.post('/setup', data={'username':USERNAME, 'password1':PASSWORD, 'password2':PASSWORD})
